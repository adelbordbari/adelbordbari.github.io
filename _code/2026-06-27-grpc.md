...
This article explains those boundaries in simple terms, shows how the code fits together, and provides safe operational procedures for maintainers. It is based on the repository as of August 2026.

## Public-safety note

This document intentionally includes architecture, filenames, service names, protocol shapes, and non-secret diagnostic commands. It does **not** reproduce private keys, certificate bodies, passwords, access tokens, database credentials, or proprietary vendor implementation details.

When a command contains `<PLACEHOLDER>`, substitute a value from the approved deployment secret store. Do not paste secrets into tickets, chat, terminal screenshots, shell history, or public logs.

## The 60-second explanation

We has two applications involved in token validation:

1. The **Django web application** is the client. Its login view calls `check_tocken()` (the misspelling is part of the current API).
2. The native **`token_server` executable** is the server. It listens on TCP port `5443`, speaks gRPC over TLS, and communicates with the underlying token/vendor library.
...
If the gRPC check succeeds according to the client's current interpretation, Django continues with normal username/password authentication. If the check fails, raises a gRPC error, or raises any other exception, the login page returns a token-disconnected message.
...
flowchart LR
    ...
    W -->|calls check_tocken| C[Python token client]
    C -->|TLS + unary gRPC LoginToken| H[Docker host gateway]
    H -->|TCP 5443| S[native token_server]
...
sequenceDiagram
    ...
    participant Client as check_tocken()
...
| `mainApp/accounting/token.py` | Builds TLS credentials, opens the secure gRPC channel, invokes `LoginToken`, interprets the response, and catches errors. | The function is named `check_tocken()`. Certificate/key material is obfuscated inline; do not expose it while debugging. |
...
| `script/install_token_server_service.sh` | Installs/enables/restarts the systemd unit and verifies port `5443` is listening. | Called during both initial installation and updates. |
...
flowchart TB
    ...
    HOST --> PORT[token_server listening on 0.0.0.0:5443]
...
```bash
docker exec web getent hosts token-service.dezhafzar.net
docker exec web sh -c 'python - <<"PY"
import socket
with socket.create_connection(("token-service.dezhafzar.net", 5443), timeout=3):
...
- [ ] Confirm port `5443` is listening.
...
- Django calls `check_tocken()` from the unauthenticated login flow.
...
- The host service listens on port `5443` and is managed by systemd.
...
Client function:    mainApp/accounting/token.py -> check_tocken()
