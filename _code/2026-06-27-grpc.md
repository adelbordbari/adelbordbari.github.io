...
The container maps the token service's TLS hostname to the Docker host gateway. That lets the client use the hostname expected by the TLS certificate while traffic reaches the host-side service.
...
```mermaid
flowchart LR
    U[User's browser] -->|HTTP login request| W[Django container]
...
- Django runs inside the container.
...
| `docker-compose.yml` | Routes the token-service hostname from the container to the host gateway. | The hostname must remain compatible with TLS identity verification. |
...
1. Docker name routing sends traffic from the container to the host.
...
Run checks from the narrowest layer outward. Most commands below require host access; commands marked “container” run inside the container.
...
- DNS/host mapping resolves from the container.
...
| Works on host, fails in container | Namespace/routing/TLS name | perform checks from inside container |
...
10. Recreate the  container only if routing/configuration changed.
...
- The container maps the service hostname to the host gateway.
